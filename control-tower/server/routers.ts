...
import { businessBuilderRouter } from "./routers/businessBuilder";
import { founderProfile, businessInterviewAnswers } from "../drizzle/schema";
  ...
  atlasChat: publicProcedure
    .input(
      z.object({
        ...
        .mutation(async ({ input }) => {
      // Fetch founder profile + Build Spec to personalize Atlas context
      let profileContext = "";
      try {
...
Always reference the founder's specific business context when giving advice. Address them by their business name, not generically. Tailor recommendations to their business type (${p.businessType || "general"}).`;
          ...
          // Inject Build Spec if available — gives Atlas full strategic context
          const interviews = await db
            .select()
            .from(businessInterviewAnswers)
            .orderBy(desc(businessInterviewAnswers.createdAt))
            .limit(1);
          if (interviews.length > 0 && interviews[0].buildSpec) {
            profileContext += `\n\nBUSINESS BUILD SPEC (generated by Business Builder OS — use this as the strategic foundation for all advice):\n${interviews[0].buildSpec.slice(0, 3000)}`;
          }
        }
...
export const appRouter = router({
  ...
  vault: vaultRouter,
  businessBuilder: businessBuilderRouter,
